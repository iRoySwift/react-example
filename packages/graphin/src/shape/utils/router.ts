...
import { getExpandedBBox, getExpandedBBoxPoint, PolyPoint, getPolylinePoints, simplifyPolyline, isSegmentCrossingBBox } from './polyline-util';
...
export interface RouterCfg {
  ...
  // eslint-disable-next-line no-unused-vars
  fallbackRoute?: (p1: PolyPoint, p2: PolyPoint, startNode?: INode, endNode?: INode, cfg?: RouterCfg) => PolyPoint[]; // 若找不到符合要求的路径时的退化路径方案
  maximumLoops?: number;
}

const manhattanDist = (p1: PolyPoint, p2: PolyPoint): number => Math.abs(p1.x - p2.x) + Math.abs(p1.y - p2.y);

const eucliDist = (p1: PolyPoint, p2: PolyPoint): number => Math.sqrt((p1.x - p2.x) ** 2 + (p1.y - p2.y) ** 2);
...
const simplePolyline = (start: PolyPoint, end: PolyPoint, startNode: INode, endNode: INode, cfg: RouterCfg) => {
...
const getBoxPoints = (
  ...
  const isInside = oriPoint.x > bbox.minX && oriPoint.x < bbox.maxX && oriPoint.y > bbox.minY && oriPoint.y < bbox.maxY;
  ...
  if (isInside) {
    ...
    for (const dir of directions) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const insterctP = Util.getLineIntersect(point, { x: point.x + dir.stepX * expandBBox.width, y: point.y + dir.stepY * expandBBox.height }, boundLine[0], boundLine[1]) as PolyPoint;
...
const getControlPoints = (current, cameFrom, scaleStartPoint, endPoint, startPoint, scaleEndPoint, gridSize) => {
...
export const pathFinder = (startPoint: PolyPoint, endPoint: PolyPoint, startNode: INode, endNode: INode, routerCfg?: RouterCfg): PolyPoint[] => {
