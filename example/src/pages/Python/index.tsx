...
import { MarkdownDocument, Tools, Wrapper } from './styled';
...
const Python: React.FC<Props> = () => {
  ...
  console.log('🚀 ~ file: index.tsx:32 ~ setBasicSetup:', setBasicSetup);
      ...
      import(`code-example/lib/${lang.toLocaleLowerCase()}.js`)
        .then((data) => {
          ...
          if (langs[lang]) {
            ...
            console.log('🚀 ~ file: index.tsx:40 ~ .then ~ color:', color, langs[lang]());
          }
          setMode(lang);
        })
        .catch(() => {
          if (langs[lang]) {
            console.log('🚀 ~ file: index.tsx:54 ~ handleLangChange ~ langs[lang]:', langs[lang]);
            setExtensions([color, langs[lang]()]);
          } else [setExtensions([color])];
          setMode(lang);
          setCode('');
        });
      if (lang == 'html') {
