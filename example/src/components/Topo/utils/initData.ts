...
import { allMenuMap } from '../model/data';
...
function initWrapperData(
  ...
  const hasNet = components.some((component: { type: string }) => component.type === 'NCM');
  // 有网络服务--新连线
  if (hasNet) {
    const ncmList = components.filter((component: { type: string }) => component.type === 'NCM');
    graphComponents = components.filter((component: { type: string }) => !['NCM', 'WAF'].includes(component.type));
    ncmList.forEach((ncm: any) => {
      let data = {
        isReal: 0
      };
      ...
      if ([1, null, undefined].includes(data.isReal)) {
        ...
        const linkObj = links.find((link: { from: any }) => [ncm.uuid, ncm.nodeId].includes(link.from));
        if (linkObj) {
          const waf = components.find((component: { type: string; uuid: any; nodeId: any }) => component.type === 'WAF' && [component.uuid, component.nodeId].includes(linkObj.to));
          if (waf) {
            graphComponents.push({
              ...waf,
              ncmUuid: ncm.uuid,
  ...
  const isErrData = graphComponents.some((c: { type: string; coordX: any; coordY: any; groupId: any }) => {
    const typeList = [isStrategy(c.type), isApp(c.type), isWeb(c.type), isNet(c.type)];
    return c.type !== 'LB' && (!c.coordX || !c.coordY) && typeList.some((t) => t) && !c.groupId;
  });
  ...
  if (isErrData || (graphInfo && graphInfo.graphInfo) || !graphInfo) {
    ...
    graphComponents.forEach((component: { type: string; groupId: any; uuid: any; nodeId: any; data: string; coordX: any; coordY: any }) => {
      const isStrategy = ['LOGBACK', 'PUBLICNETAGENT', 'SPM', 'LOGCOLLECT', 'AtlasSLGS'].includes(component.type);
      const service = {
      ...
      if (!isStrategy) {
        ...
        const isGroup = !isStrategy && ![2, '2'].includes(service.showLocation);
        if (isGroup) {
          appGroup = {
            collapsed: false,
            type: 'app',
    ...
    // // 策略类节点
    const strategyNodes = graphComponents.filter((component: { type: string }) => ['LOGBACK', 'PUBLICNETAGENT', 'SPM', 'LOGCOLLECT', 'AtlasSLGS'].includes(component.type));
    strategyNodes.forEach((strategyNode: { uuid: any; nodeId: any; type: string | number }) => {
      const linkObj = graphLinks.find((link) => [strategyNode.uuid, strategyNode.nodeId].includes(link.to));
      const service = {
      ...
      if (linkObj) {
        ...
        const fromNode: any = nodes.find((node) => [node.id, node.uuid].includes(linkObj.from));

        const minX = fromNode.x;
        const maxY = fromNode.y + 100;

        const appGroup: any = groups.find((group) => group.id === fromNode.parent);
        const appStrategys = graphLinks.filter((link) => link.type === 2 && link.from === linkObj.from && strategyNodes.some((node: { nodeId: any; uuid: any }) => [node.nodeId, node.uuid].includes(link.to)));
        const strategyIndex = appStrategys.findIndex((link) => [strategyNode.uuid, strategyNode.nodeId].includes(link.to));
        const node = {
          x: minX,
    ...
    let _graphLinks: any[] = [];
    try {
      _graphLinks = (graphInfo.graphInfo && JSON.parse(graphInfo.graphInfo).edges) || [];
    } catch (e) {
      console.log(e);
      _graphLinks = [];
    }
    graphLinks.forEach((link: { from: any; to: any; type?: any }) => {
      // eslint-disable-next-line no-underscore-dangle
      const _link = _graphLinks.find((item) => item.from === link.from && item.to === link.to);
      let sourceAnchor = 1;
      let targetAnchor = 0;
      if (_link) {
