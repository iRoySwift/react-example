...
import { Decoration, ViewPlugin, DecorationSet, EditorView, ViewUpdate, PluginValue } from '@codemirror/view';
...
function createGradient(markerCssProperty: string, indentWidth: number, startOffset: number, columns: number) {
  ...
  return `${gradient} ${startOffset * indentWidth}.25ch/calc(${indentWidth * columns}ch - 1px) no-repeat`;
}
